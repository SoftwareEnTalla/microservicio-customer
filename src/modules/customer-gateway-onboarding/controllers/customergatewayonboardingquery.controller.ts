...
import { CustomerGatewayOnboardingDto } from "../dtos/all-dto";
...
@ApiTags("CustomerGatewayOnboarding Query")
@Controller("customergatewayonboardings/query")
export class CustomerGatewayOnboardingQueryController {
  #logger = new Logger(CustomerGatewayOnboardingQueryController.name);

  constructor(private readonly service: CustomerGatewayOnboardingQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all customergatewayonboarding with optional pagination" })
  @ApiResponse({ status: 200, type: CustomerGatewayOnboardingsResponse })
  @ApiQuery({ name: "options", required: false, type: CustomerGatewayOnboardingDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CustomerGatewayOnboarding>    
  ): Promise<CustomerGatewayOnboardingsResponse<CustomerGatewayOnboarding>> {
    try {
     
      const customergatewayonboardings = await this.service.findAll(options);
      logger.info("Retrieving all customergatewayonboarding");
  ...
  @ApiOperation({ summary: "Find customergatewayonboarding by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter customergatewayonboarding', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CustomerGatewayOnboardingsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CustomerGatewayOnboarding no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find customergatewayonboardings with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CustomerGatewayOnboarding>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all customergatewayonboardings" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count customergatewayonboardings with conditions" })
  @ApiResponse({ status: 200, type: CustomerGatewayOnboardingsResponse<CustomerGatewayOnboarding> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades CustomerGatewayOnboardings no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one customergatewayonboarding with conditions" })
  ...
  @ApiOperation({ summary: "Find one customergatewayonboarding or return error" })
  @ApiResponse({ status: 200, type: CustomerGatewayOnboardingResponse<CustomerGatewayOnboarding> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad CustomerGatewayOnboarding no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get customergatewayonboarding by ID" })
  @ApiResponse({ status: 200, type: CustomerGatewayOnboardingResponse<CustomerGatewayOnboarding> })
  @ApiResponse({ status: 404, description: "CustomerGatewayOnboarding not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the customergatewayonboarding to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CustomerGatewayOnboardingResponse<CustomerGatewayOnboarding>> {
    try {
      const customergatewayonboarding = await this.service.findOne({ where: { id } });
      if (!customergatewayonboarding) {
        throw new NotFoundException(
          "CustomerGatewayOnboarding no encontrado para el id solicitado"
        );
      }
      return customergatewayonboarding;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
