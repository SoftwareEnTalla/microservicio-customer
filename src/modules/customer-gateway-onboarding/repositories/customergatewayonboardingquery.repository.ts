...
import { logger } from '@core/logs/logger';
  ...
  @Injectable()
  export class CustomerGatewayOnboardingQueryRepository {

    //Constructor del repositorio de datos: CustomerGatewayOnboardingQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination CustomerGatewayOnboarding on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[CustomerGatewayOnboarding[], number]> {
      logger.info('Ready to findAndCount CustomerGatewayOnboarding on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of CustomerGatewayOnboarding on repository:',result);
      return result;
    }
    ...
    client: LoggerClient.getInstance()
      ...
      .get(CustomerGatewayOnboardingRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<CustomerGatewayOnboarding | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) CustomerGatewayOnboarding:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<CustomerGatewayOnboarding> = options as FindOptionsWhere<CustomerGatewayOnboarding>;
