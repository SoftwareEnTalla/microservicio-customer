...
@Injectable()
export class CustomerGatewayOnboardingService {
  constructor(private readonly repository: CustomerGatewayOnboardingRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<CustomerGatewayOnboarding[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<CustomerGatewayOnboarding[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<CustomerGatewayOnboarding[]> {
  ...
  async update(id: string, partialEntity: Partial<CustomerGatewayOnboarding>): Promise<CustomerGatewayOnboarding | null> {
