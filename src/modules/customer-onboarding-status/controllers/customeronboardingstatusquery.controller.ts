...
import { CustomerOnboardingStatusDto } from "../dtos/all-dto";
...
@ApiTags("CustomerOnboardingStatus Query")
@UseGuards(CustomerOnboardingStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("customeronboardingstatuss/query")
export class CustomerOnboardingStatusQueryController {
  #logger = new Logger(CustomerOnboardingStatusQueryController.name);

  constructor(private readonly service: CustomerOnboardingStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all customeronboardingstatus with optional pagination" })
  @ApiResponse({ status: 200, type: CustomerOnboardingStatussResponse })
  @ApiQuery({ name: "options", required: false, type: CustomerOnboardingStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CustomerOnboardingStatus>    
  ): Promise<CustomerOnboardingStatussResponse<CustomerOnboardingStatus>> {
    try {
     
      const customeronboardingstatuss = await this.service.findAll(options);
      logger.info("Retrieving all customeronboardingstatus");
  ...
  @ApiOperation({ summary: "Find customeronboardingstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter customeronboardingstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CustomerOnboardingStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CustomerOnboardingStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find customeronboardingstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CustomerOnboardingStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all customeronboardingstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count customeronboardingstatuss with conditions" })
  @ApiResponse({ status: 200, type: CustomerOnboardingStatussResponse<CustomerOnboardingStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one customeronboardingstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one customeronboardingstatus or return error" })
  @ApiResponse({ status: 200, type: CustomerOnboardingStatusResponse<CustomerOnboardingStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get customeronboardingstatus by ID" })
  @ApiResponse({ status: 200, type: CustomerOnboardingStatusResponse<CustomerOnboardingStatus> })
  @ApiResponse({ status: 404, description: "CustomerOnboardingStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the customeronboardingstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CustomerOnboardingStatusResponse<CustomerOnboardingStatus>> {
    try {
      const customeronboardingstatus = await this.service.findOne({ where: { id } });
