...
@Controller("customers/query")
export class CustomerQueryController {
  #logger = new Logger(CustomerQueryController.name);

  constructor(private readonly service: CustomerQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all customer with optional pagination" })
  @ApiResponse({ status: 200, type: CustomersResponse })
  @ApiQuery({ name: "options", required: false, type: CustomerDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Customer>    
  ): Promise<CustomersResponse<Customer>> {
    try {
     
      const customers = await this.service.findAll(options);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the customer to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CustomerResponse<Customer>> {
    try {
      const customer = await this.service.findOne({ where: { id } });
      if (!customer) {
  ...
  @ApiOperation({ summary: "Find customer by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter customer', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CustomersResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Customer no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find customers with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Customer>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all customers" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count customers with conditions" })
  @ApiResponse({ status: 200, type: CustomersResponse<Customer> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one customer with conditions" })
  @ApiResponse({ status: 200, type: CustomerResponse<Customer> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiOperation({ summary: "Find one customer or return error" })
  @ApiResponse({ status: 200, type: CustomerResponse<Customer> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
