...
import { logger } from '@core/logs/logger';
  ...
  export class CustomerQueryRepository {
    ...
    //Constructor del repositorio de datos: CustomerQueryRepository
    constructor(
    ...
    async findWithPagination(
      ...
      logger.info('Ready to findWithPagination Customer on repository:', options);
      return this.repository.find(options);
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Customer[], number]> {
      logger.info('Ready to findAndCount Customer on repository:',where);
      let result= this.repository.findAndCount({
        where: where,
      });
      logger.info('Was counted  instances of Customer on repository:',result);
      return result;
    }
    ...
    async findOne(where?: Record<string, any>): Promise<Customer | null> {
      const tmp: FindOptionsWhere<Customer> = where as FindOptionsWhere<Customer>;
      logger.info('Ready to findOneBy Customer on repository with conditions:', tmp);
